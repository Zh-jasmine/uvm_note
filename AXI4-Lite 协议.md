...
##  CSR （Control and Status Register 控制状态寄存器）or 配置寄存器组

这个 DUT 是一个 AXI-to-SPI Bridge，它内部所有寄存器的功能都围绕着 SPI 工作参数展开。CPU 原生支持 AXI 协议，不需要额外的协议翻译就能发起总线事务，但 AXI 只负责运输数据，不负责理解数据的语义。寄存器层承担了语义转换的角色：AXI 接口将 CPU 发来的地址和数据正确接收后，寄存器层将其翻译成 SPI master 需要的具体配置（如 CPOL/CPHA 模式、时钟分频、帧间隙时序等）。每个寄存器有独立的地址和功能——有的控制 SPI 启动（slv_reg0），有的查询忙状态（slv_reg1），有的发送数据（slv_reg8），有的接收数据（slv_reg9）。寄存器层还提供了频率适配的缓冲作用：CPU 在 GHz 级别完成写入后可以立刻处理其他事务，SPI 在 MHz 级别逐 bit 串行移位由 SPI master 独立完成。

AXI4-Lite 按字节地址寻址。这个 DUT 的地址总线宽度是 6bit（有六根线），最多可以寻址 64 字节（$2^6$）。

补充：两根线，一共产生四个地址：00、01、10、11；
在CPU 设计里约定好了：每个地址对应 8 bit（1 字节）。Intel 8086、ARM、RISC-V……全世界的 CPU 都认这个标准。这个约定从 1960 年代的 IBM System/360 开始，一直延续到现在。
所以6根线能产生（$2^6$）64个地址，等于寻址64字节。

数据总线宽度是 32bit（4 字节），意味着每次传输读取或写入 4 字节。
所以RTL中定义的寄存器组（存储数据）是4字节宽度，1地址访问1字节。
所以
```
slv_reg0 → 占了地址 0、1、2、3
slv_reg1 → 占了地址 4、5、6、7
slv_reg2 → 占了地址 8、9、10、11
```
这就要求cpu的读写地址必须是0、4、8，4的倍数，表示成二进制就是最低两位是0。

RTL 中通过两个参数实现这个映射：ADDR_LSB 设为 2，表示跳过最低的 2 个字节地址位；OPT_MEM_ADDR_BITS 设为 3，表示用 3bit 来选择 8 个 32bit 寄存器。（译码器）
