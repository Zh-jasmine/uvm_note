...
经过与多位从业者的讨论，确定了当前方向。以下几个候选方案被评估后排除：

- **AES-128 加密核**：通用性偏低，主要面向 crypto 领域；优点是可以同时做设计和验证，但算法验证的思路在 SoC 级岗位中复用度有限
- **AHB-to-APB Bridge**：培训班最经典的案例，面试官已经视觉疲劳，简历中出现可能直接 pass
- **Cache 控制器**：复杂度偏高，作为第一个项目容易烂尾，验证闭环难以完成
- **AXI4-Lite Slave 纯寄存器模块**：结构清晰但 DUT 太薄，逻辑含量低，更像 UVM 练习而非真实项目

AXI4-Lite to SPI Bridge 同时覆盖总线协议和功能逻辑，验证点丰富但规模可控，属于"中等复杂控制型 IP"。
